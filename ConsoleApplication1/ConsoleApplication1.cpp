...
STAGE command_execut(void)
{
	...
	uint32_t i;
	uint32_t size = 0, size_e = 0, size_b = 0;
	...
	case 3: // Clear Page  - вычитать сектора в буфер, очистить их, записать флэш по адресам вне страницы из буфера
	{
		...
		size_b = transData.flash_address_start - transData.flash_sector_address_start;
		size_e = transData.flash_sector_address_end - transData.flash_address_end;
		...
		size = transData.flash_address_end - transData.flash_sector_address_start + 4;/////////////////////////////////
		...
		for (ind = transData.sector_start; ind <= transData.sector_end; ind++)
		{
			...
			{
//				printf(" send CS%d\n", ind);////////////////////////////////////////////
				if(send_cmd_CS(ind, ind) ==0)printf("Clear Sector %d is OK\n", ind); 
			}
			else printf("Sector %d is empty\n", ind);
		}
		// восстановление участков вне страницы очистки/ добавить проверку границ восстановления
		if(size_b&&sect_busy[transData.sector_start])
		{ 
		...
		if (size_e&&sect_busy[transData.sector_end])
		{ 
	...
	case 5: // write sector - работает//
	{
		...
		{
			if (send_cmd_CS(transData.sector_start, transData.sector_start) == 0)
			{
...
//		ind;
		...
		printf("%s\n", filename);
		flash_data_buffer = (uint8_t*)malloc(size);
		// Вычитываем содержимое flash в секторах, содержащим страницу и сохраняем в page_buffer_file
		send_cmd_RP(transData.flash_sector_address_start, transData.flash_sector_address_end,/* flash_data_buffer, */"page_buffer_file");
		readMSG(4);
		if (verifMSG((uint8_t*)"EOF", rx_buff, 4) != 0)
		{
			error = (uint32_t)rx_buff;
			printf("Error of buffering sectors= %d\n", error);
		}
		fclose(pFile);
		size_b = transData.flash_address_start - transData.flash_sector_address_start;	//////////вынести в начало функции////////////////////////////////////
		// проверяем на необходимость очистки секторов, содержащих страницу
		...
		// формируем буфер с начала сектора до начала страницы
		recovery_buff_begin = (uint8_t*)malloc(size_b);
		if (size_b)
		{
			printf("Creat recovery Buffer_begin:\n");
...
//			printf("\n");////////////////////////////////////////////////////////////
		...
		// формируем буфер с конца страницы до конца сектора
		...
		if (size_e)
		{
			...
			printf("Creat recovery Buffer_end:\n");
			i = 0;
		...
		free(flash_data_buffer);
		// готовим данные для записи страницы
...
uint8_t sector_form(char* buf, uint8_t* data, uint8_t size)////////////////исправить интерпритацию ->sector_end
{
	uint8_t i = buf - &buff[0];
	char st[2] = { 0 };
	i = find_separ(i + 1, size - 1);
